...
_(Work toward next release. Currently under development.)_
...
* Fixed issue that allowed access to `list_users` API endpoint in some cases
  when it should not have been allowed
...
* Updated several question types to bold the word "Solution" in answer displays
  so they all match

**FIXED**

* Fixed an issue with broken save buttons

* Fixed an issue with incorrect alignment of `multiplechoice` questions'
  answers in browsers other than Firefox
...
* Added the option to specify solutions to `fileupload` questions with Markdown
  code instead of files

**FIXED**

* Fixed a crash when a "checkbox"-style `multiplechoice` question had no `True`
  answers

* Fixed a UI issue whereby darkmode preferences were inherited when
  impersonating a student

* Replaced code that used the `imp` module, which has been removed from Python
  3.12

* Updated handling of diagrams so that they respect darkmode settings

* Fixed a broken implementation of solutions in `fileupload` questions

* Fixed a rare crash with asynchronous checkers not always making sure the
  appropriate directories exist before attempting to write log files

* Fixed an issue with password-locked pages that prevented them from ever being
  accessed.

**VENDORED SOFTWARE**

* A version of `cgi.py` from the Python standard library is now included in the
  distribution, since that module will be removed from the standard library in
  3.13 and we depend on it for parsing form data.
...
* Fixed a regression that caused footnotes not to be displayed
...
* Added a workaround for `BeautifulSoup` parsing `<br/>` tags strangely
...
* The `fileupload` question type no longer alerts when an empty file is
  specified; instead, the submission is simply treated as empty

**FIXED**

* Fixed a bug with questions that are re-rendered after submission

* Fixed issues with handling of questions with `csq_indiv=False` in the
  submission callback
...
* Changed size of per-user random seeds

* Redesigned interface for staff for adjusting per-user random seeds

**FIXED**

* Fixed crash due to form values being specified multiple times
...
* If `cs_sole_course` is set to a course's name, the root URL will now refer to
  that course directly (with no redirect)

* Added additional arguments to the callback function for `ajaxrequest` in the
  default handler

* The `pythoncode` question type now listens to the `csq_show_check` flag

* Added the `download` and `filename` options to the `raw_response` handler to
  allow making the responses downloadable (thanks to Shen Shen)

* `<python>` tags can now be exited using `sys.exit()`, with non-zero return
  codes causing the output to be ignored (or with a string value, using that
  instead of the Python block's output

* Additional information is now stored in the `question_info` logs

* Added the ability for users to specify that they prefer light text on a dark
  background

* Updates to the question info cache can now be avoided by setting
  `cs_update_questions_cache = False`

* Added links to leave user settings pages, back to the most recent
  non-settings page viewed
...
* Changed location of lock files to (hopefully) prevent the size of the locks
  directory from growing out of control

* Javascript files are now located at `/js/` instead of `/scripts/`

* `<chapter>`, `<section>`, `<subsection>`, and `<subsubsection>` tags have
  been replaced with `<catsoop-chapter>`, `<catsoop-section>`, etc; and a
  conversion script has been provided

* Moved the default `remote` Python sandbox and improved the error message when
  the sandbox is offline.

* Changed the following special variable names to make their purpose clearer:
...
* Removed the `cert` login type.

**FIXED**

* Fixed a crash from `render_escaped_dollar` method not existing (triggered by
  `mistletoe` update?)

* Fixed multiple issues with `-c` flag and `CATSOOP_CONFIG` environment
  variable

* Fixed incorrect help text with `catsoop logedit` command

* Fixed an old bug where submitting an asynchronously-checked question while
  another submission is queued for that same question caused confused results
  to be displayed

* Fixed a potential bug in the `pythoncode` question type when decoding
  submitted files containing a BOM

* An empty string is no longer considered to be a well-formed response to a
  `pythonic` question

* Fixed a potential bug with the length calculation in the `raw_response`
  header when `response` was given as a string containing unicode characters
  (thanks to Shen Shen)

* Fixed an issue with custom `csq_check_function` used with `checkbox`
  renderer in `multiplechoice` questions.

* Fixed a display issue causing a "hamburger" to appear on narrow screens
  even with no menu items present

* Replaced the `<showhide>` implementation so as to avoid broken rendering of
  `<details>` tags in Chromium/Chrome

* Code blocks within responses to question submissions are now properly
  highlighted

* Fixed a regression that caused the `timeout` option in `pythoncode` test
  cases to be ignored

* Fixed several issues with submission data not being handled properly
  (potentially breaking pages after 'save' or 'view answer')

* Fixed misleading error message in `expression` questions with
  `csq_error_on_unknown_variable` set to `True` and multiple solutions

**SECURITY**

* Several authentication types have been fixed to pull authentication
  information from a context that cannot be altered by individual courses.

**ACCESSIBILITY**

* Changed `radio` and `checkbox` multiplechoice variants to use `fieldset`s and
  to properly associate labels with inputs

* Improved screen-reader-friendliness of solution displays for the `checkbox`
  multiplechoice renderer

* Improved code displays (with and without line numbers) to make them more
  usable with a screen reader

* Automatically set ARIA labels for built-in question types based on
  `csq_prompt` (and added the ability to specify a `csq_preamble` that is not
  included as part of the ARIA label).

* Switched from KaTeX to MathJax for math rendering, for better screen reader
  support

* Improved keyboard-oriented navigation and screen-reader support for the
  navigation breadcrumbs and the top menu

* Improved accessibility of modal dialogs from the `cs_modal` javascript
  function

**VENDORED SOFTWARE**

* Removed KaTeX from the distribution

* Included [MathJax](https://www.mathjax.org/) (v3.2.2) in the distribution

* Upgraded [CodeMirror](https://codemirror.net/) to version 6


# Version 18.0.0

_(formerly known as 2022.9.0)_

**ADDED**

* Added `cs_log_lock_location` to control where locks for log files are written

* Added `csq_renderer` option to `pythonic` qtype, to allow choosing between
  `bigbox` and `smallbox` renderers

* The `pythonic` qtype now respects the `csq_check_function` variable (which
  can still be overridden by specifying check functions for individual tests)

* Questions with a "Check" button now have a "Revert to Prior Submission"
  button

* Added `cs_wsgi_server_worker_max_requests` option to provide `--max-requests`
  option to uWSGI, to force workers to be killed and restarted after that
  many requests

**CHANGED**

* `cs_local_python_import` now raises a more meaningful exception when the
  given filename is incorrect

* Replaced hacky import mechanism for custom question types with access to
  `cs_local_python_import`

* Questions with a "Check" button no longer also have a "Save" button

* Added error reporting on failed `<include>` tags
...
* Fixed an issue with `checkoff` question type not understanding who has
  permissions to submit checkoffs

* Fixed an issue with storage of manual grades

* Fixed an error with certain LTI integrations

* Fixed an incorrect check for whether the target(s) of `<include>` tags are
  within the tree

* Fixed an issue with `checker_local`'s `multiprocessing` on MacOS

* Every page now loads the CodeMirror base code, to fix an error caused by
  multiple code-like questions using different CodeMirror modes but bringing
  in a completely fresh `codemirror.js` for each one (overriding the old modes)

* Replaced deprecated `get_event_loop` function in reporter.py script
...
* `cs_auth_type` is now only read from `config.py` (course-level `preload.py`
  files cannot change it)

**VENDORED SOFTWARE**

* Upgraded [Codemirror](https://codemirror.net/) to v5.65.6 (upgrating to
  Codemirror v6 will happen in a later catsoop version)

* Upgraded [KaTeX](https://katex.org/) to v0.16.0

* Upgraded [Twemoji](https://github.com/twitter/twemoji) to v14.0.2


# Version 17.0.0

_(formerly known as 2021.9.0)_
...
* Logging of page views (in the `problemactions` log) can now be enabled by
  setting `cs_log_page_views = True`
...
* The `pythonic` and `pythonliteral` question types now allow checking for
  valid formatting
...
* Replaced `csq_grading_mode="legacy"` with `csq_autograder_async` flag, and
  made synchronous checking the default
...
* Upgraded [Twemoji](https://github.com/twitter/twemoji) to v13.1.0
...
_(formerly known as 2021.2.0)_
...
_(formerly known as 2020.9.0)_
...
* Replaced `js_files` in question types with `extra_headers`, for injecting
  arbitrary strings into the header (which allows specifying not only
  Javascript files but also stylesheets)
...
_(formerly known as 2020.2.0)_
...
_(formerly known as 2019.9.6)_
...
_(formerly known as 2019.9.5)_
...
_(formerly known as 2019.9.4)_
...
_(formerly known as 2019.9.3)_
...
_(formerly known as 2019.9.2)_
...
_(formerly known as 2019.9.1)_
...
_(formerly known as 2019.9.0)_

**ADDED**

* Checking functions for `pythoncode` questions can now optionally return a
  tuple `(score, message)` or a dictionary `{'score': score, 'msg': message}`
  to display an additional message in response to a test case.
...
* CAT-SOOP will use date-based versioning moving forward.  Planned regular
  releases are `20XX.9` and `20XX.2` (September and February, corresponding
  with the start of the fall and spring terms at MIT, respectively).  Each
  release will also now have a codename.
...
* Removed several deprecated features:
   ...
   * `csq_multiplechoice_renderer` and `csq_multiplechoice_soln_mode` variables
     for multieplchoice questions have been renamed to `csq_renderer` and
     `csq_soln_mode`, respectively.
...
* Fixed an issue with the checker process, whereby submissions would forever
  ...
  when looking up the question that was submitted.
...
* Several additional features were added to `pythoncode` questions:
   ...
   * Added the option to selectively show/hide stderr via
     `test['show_stderr']`.
...
* The CAT-SOOP source distribution now includes the [Ace](https://ace.c9.io/)
  code editor.
...
* Updated [KaTeX](https://katex.org/) to version 0.10.0.
...
* Changed the structure of JS web labels for
  [LibreJS](https://www.gnu.org/software/librejs/), to make it clearer
...
* Fixed a type issue related to `mpmath` objects in `numpy` arrays.
...
* Fixed an issue with `expression` question types, whereby if all values were
  ...
  likely `int`s or `float`s (instead of `mpmath.mpf`) to represent them,
...
* Made CAT-SOOP work properly with
  [LibreJS](https://www.gnu.org/software/librejs/).

* Added support for Python sandboxing via
...
* Updated [KaTeX](https://katex.org/) to version 0.10.0-rc.1.
...
* "Log In" and "Log Out" no longer discard the query string associated with
  the current page (courtesy of Jeremy Wright).
...
* Upgraded [KaTeX](https://katex.org/) to version 0.9.0.
...
* Upgraded [PLY](https://www.dabeaz.com/ply/index.html) to version 3.11.
...
* In the default handler, ensure that `handle_check` creates all `problemstate`
...
* Updated [KaTeX](https://katex.org/) to 0.9.0-beta1.
...
* Fixed issues with the `circuit` question type that prevented an AC analysis
  from being run interactively, and that caused incorrect AC results to be sent
  to the checker.
...
* Fixed syntax error (typo) in `handout` handler.
...
* Removed more traces of the abandoned queue.
...
* Updated [KaTeX](https://katex.org/) to version 0.8.2.
...
* Updated [MathJax](https://www.mathjax.org/) to version 2.7.1, and updated it
  to use a future-proof renderer.

* Updated [PLY](https://www.dabeaz.com/ply/index.html) to version 3.10.

* Updated Markdown to version 2.6.8.

* Updated [BeautifulSoup](https://www.crummy.com/software/BeautifulSoup/) to
  version 4.6.0.
...
* Updated [KaTeX](https://katex.org/) to version 0.7.0
...
* Fixed issue related to improper handling of empty `<python>` or `<question>`
  tags.
...
* Improved the formatting of HTML in the `pythoncode` question type to
  prevent BeautifulSoup from modifying it too much.
...
* Fixed a typo in `dispatch.py` that broke proper 404 handling of handouts.
...
* Added [BeautifulSoup](https://www.crummy.com/software/BeautifulSoup/) to the
  distribution.
...
* Added support for authentication via [OpenID
...
* A little easter egg: the CAT-SOOP cat changes when displaying a 404 or 500
  error message.
...
* Updated [MathJax](https://www.mathjax.org/) to version 2.6.1 to fix a
...
* Fixed an issue in the `pythoncode` sandboxes whereby a `MEMORY` limit of 0
...
* Fixed a bug that allowed logs to be accessed/created outside of the
  appropriate `__LOGS__` directories.
...
* Rewrote the `expression` question type to use
  [PLY](https://www.dabeaz.com/ply/index.html) for parsing, and included
...
* Added support for [Markdown](https://daringfireball.net/projects/markdown/)
  ...
  [python-markdown](https://pypi.python.org/pypi/Markdown) in the distribution.
...
* Added a `'string'` mode to the `pythonic` question type, which allows the
  ...
  `csq_code_pre` variable to this question type, for setting up the environment
  into which csq_soln will be evaluated in string mode.
...
* Mostly embraced [PEP8](https://www.python.org/dev/peps/pep-0008/) style.
...
* Added `post_load` hook, which is executed after the content file is loaded.
...
* Removed several hard-coded references to `sicp-s2.mit.edu` in the code.
...
* Added support for the [ACE](https://ace.c9.io/) code editor in `pythoncode`
  questions.
...
* Separated loading from `METADATA.py` into `EARLY_LOAD.py` and `LATE_LOAD.py`.
  ...
  associated with the leaf node is executed (to allow some code execution to be
  avoided when working down the tree).
