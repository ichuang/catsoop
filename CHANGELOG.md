...
* The `catsoop.tools` library was renamed to `catsoop.thirdparty` to better reflect its contents.  The `csm_tools` pointer to these libraries will be removed in a future version.  Use `csm_thirdparty` instead.  Many of the libraries themselves were also removed (and should be imported directly, rather than accessed via `csm_thirdparty`)
...
* Added Beautiful Soup to the distribution.  (<https://www.crummy.com/software/BeautifulSoup/>)
...
* Question types can now have multiple form fields by having names starting with `__QNAME_`, where `QNAME` is the name of the question.
...
* PBKDF2 (https://en.wikipedia.org/wiki/PBKDF2) is now used for the `'login'` authentication mode.
...
* Added `cs_util` resources: `time`, which yields the current time (according to the server) for synchronization purposes; `source.zip`, which downloads a zip archive containing the CAT-SOOP source code; and `license`, which contains the text of CAT-SOOP's license.
...
* Added support for the [ACE code editor](https://ace.c9.io/#nav=about) in `pythoncode` questions.
