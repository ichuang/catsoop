...
try:
    ...
    assert os.path.isdir(log_backup_loc) and "_queues" in os.listdir(log_backup_loc)
except:
...
if os.path.isdir(uploads_dir):
    ...
    for ix, u in enumerate(uploads):
        with open(os.path.join(uploads_dir, u, "info"), "rb") as f:
            info = pickle.load(f)
        with open(os.path.join(uploads_dir, u, "content"), "rb") as f:
            content = f.read()
...
def _transform_submission(sub):
    if isinstance(sub, list):
        return {"type": "file", "name": sub[0], "id": upload_id_map[sub[1]]}
    else:
...
checker_dir = os.path.join(log_backup_loc, "_queues", "checker", "results")
if os.path.isdir(checker_dir):
    print("Converting checker results")
    results = sorted(os.listdir(checker_dir))
