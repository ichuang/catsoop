...
class LTIOAuthServer(oauth2.Server):
    ...
    def lookup_consumer(self, key):
        ...
        if not secret:
            log.critical(
                (
                    "Consumer %s, is missing secret"
                    "in settings file, and needs correction."
                ),
                key,
            )
            return None
...
def _post_patched_request(consumers, lti_key, body, url, method, content_type):
    ...
    response, content = client.request(
        url, method, body=body.encode("utf-8"), headers={"Content-Type": content_type}
    )
...
def post_message2(
    consumers, lti_key, url, body, method="POST", content_type="application/xml"
):
    """
...
def verify_request_common(consumers, url, method, headers, params):
    ...
    oauth_server = LTIOAuthServer(consumers)
    oauth_server.add_signature_method(SignatureMethod_PLAINTEXT_Unicode())
    oauth_server.add_signature_method(SignatureMethod_HMAC_SHA1_Unicode())
...
def generate_request_xml(message_identifier_id, operation, lis_result_sourcedid, score):
        ...
        for key, value in self.items():
            ...
            if isinstance(value, STRING_TYPES):
                ...
                try:
                    ...
                    items.append(
                        (oauth2.to_utf8_if_string(key), oauth2.to_utf8_if_string(value))
                    )
                else:
                    items.extend(
                        (oauth2.to_utf8_if_string(key), oauth2.to_utf8_if_string(item))
                        for item in value
                    )
...
class LTIBase(object):
    ...
    def post_grade(self, grade):
        ...
        if 0 <= score <= 1.0:
            xml = generate_request_xml(
                message_identifier_id, operation, lis_result_sourcedid, score
            )
            ret = post_message(self._consumers(), self.key, self.response_url, xml)
            if not ret:
    ...
    def post_grade2(self, grade, user=None, comment=""):
        ...
        lti2_url = self.response_url.replace(
            "/grade_handler", "/lti_2_0_result_rest_handler/user/{}".format(user)
        )
        score = float(grade)
        if 0 <= score <= 1.0:
            body = json.dumps(
