...
def list_all(cmp_func=None):
    ...
    return lambda sub, soln: (len(sub) == len(soln) and
                              all(cmp_func(i, j) for i, j in zip(sub, soln)))
...
def dict_all(cmp_func=None):
    cmp_func = cmp_func or equal()
    return lambda sub, soln: (set(sub) == set(soln) and
                              all(cmp_func(sub[i], soln[i]) for i in sub))
