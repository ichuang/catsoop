...
def cs_ajax_secret(username):
    """
    A function mapping from usernames to "secret" keys.  These should be hard
    to guess given the username that is inputted (i.e., you should override
    this function in C{config.py}, and ideally again within each course's
    preload).
    """
    return str(hash(username))
...
def cs_debug(value, tag=''):
    ...
    with open(cs_debug_log_location, 'a') as myfile:
        print >> myfile, datetime.now().time(), tag, value

# Checks for valid configuration

_cs_config_errors = []

# store original cs_ajax_secret so we can make sure it has been
# overridden later on.
_cs_ajax_secret_orig = cs_ajax_secret
