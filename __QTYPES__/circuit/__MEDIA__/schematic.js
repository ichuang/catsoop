 ...
 *
 *
 * The original code is part of edX Platform (https://github.com/edx/edx-platform)
 * Copyright (C) 2011 Massachusetts Institute of Technology
 * Available under the GNU Affero General Public License, version 3.
 * Original:
 ...
 *     * Changed 'L' -> 'b' for label so all part types are lower-case.
 *     * Added 'silent' parameter to analyses to prevent output.
 *     * Modified output of AC analysis to contain more
 *       information when stringified.
 *     * Added prepare_submission function to run simulations and
 *       update the JSON in the hidden form element.
 *     * Added catsoop_transient_analysis function to mirror structure of
 *       ac_analysis function.
 */


var cktsim = (function() {
    // for information about modified nodal analysis (MNA) stamps, see
    ...
    Circuit.prototype.load_netlist = function(netlist, silent) {
        ...
        if (type == 'view' || type == 'w' || type == 'g' || type == 's' || type == 'b') {
            continue;
        }
    ...
    Circuit.prototype.tran = function(ntpts, tstart, tstop, probenames, no_dc, silent) {
        ...
        if ((this.diddc == false) && (no_dc == false)) {
            if (this.dc(silent) == undefined) { // DC failed, realloc mats and vects.
                if (!silent){
    ...
    Circuit.prototype.ac = function(npts,fstart,fstop,source_name, silent) {
        silent = (typeof(silent) == 'undefined') ? false : silent;
        ...
        if (this.device_map[source_name] === undefined) {
            ...
            return 'AC analysis failed, unknown source ' + JSON.stringify(source_name);
        }
    ...
    function parse_source(v) {
        ...
        // sin(v_offset,v_amplitude,freq_hz,t_delay,phase_offset_degrees)
            ...
            src.value = function(t) {  // closure
                if (t < td)
                    return voffset + va*Math.sin(2*Math.PI*phase);
                else
...
schematic = (function() {
    ...
    function Schematic(input) {
        ...
        if (parts.length == 0 && analyses.length == 0)
            this.diagram_only = true;
        else
    ...
    Schematic.prototype.help = function() {
        // TODO: where will this live?  can we even fill this in?
    ...
    Schematic.prototype.prepare_submission = function() {
        ...
        for (var i in this.submit_analyses){
            ...
            if (i == 'ac'){
                this.ac_analysis(
                    this.input.getAttribute('ac_npts') || this.ac_npts,
                    this.input.getAttribute('ac_lo') || this.ac_fstart,
                    this.input.getAttribute('ac_hi') || this.ac_fstop,
                    this.input.getAttribute('ac_name') || this.ac_source_name,
                    true);
            }else if (i == 'tran'){
                this.catsoop_transient_analysis(
                    this.input.getAttribute('tran_npts') || this.tran_npts,
                    this.input.getAttribute('tran_tstop') || this.tran_tstop);
            }else{
                this.dc_analysis(true);
            }
        }
        this.update_value();
    }
    ...
    Schematic.prototype.json_with_analyses = function() {
        ...
        if (this.dc_results != undefined)
            json.push(['dc',this.dc_results]);
        if (this.ac_results != undefined)
            json.push(['ac',this.ac_results]);
        if (this.transient_results != undefined)
            json.push(['transient',this.transient_results]);
    ...
    Schematic.prototype.extract_circuit = function(silent) {
        silent = (typeof(silent) == 'undefined') ? false : silent;
        ...
        if (ckt.load_netlist(netlist, silent))
            return ckt;
        else
            return null;
    }

    Schematic.prototype.dc_analysis = function(silent) {
        silent = (typeof(silent) == 'undefined') ? false : silent;
        // remove any previous annotations
        this.unselect_all(-1);
        this.redraw_background();

        var ckt = this.extract_circuit(silent);
        if (ckt === null){
            this.dc_results = null;
            return;
        }
    ...
    Schematic.prototype.ac_analysis = function(npts,fstart,fstop,ac_source_name,silent) {
        silent = (typeof(silent) == 'undefined') ? false : silent;
        var ckt = this.extract_circuit();
        if (ckt === null){
            this.ac_results = null;
            return;
        }
        var results = ckt.ac(npts,fstart,fstop,ac_source_name, silent);
                ...
                if (submit != undefined) {
                    ...
                    for (var i in ckt.node_map){
                        ...
                        if (results[i] instanceof Array){
                            ...
                            var m = new Array(results._frequencies_.length).fill(results[i]);
                            var p = new Array(results._frequencies_.length).fill(results[pk]);
                        }
                        this.ac_results[i]['magnitude'] = m;
                        this.ac_results[i]['phase'] = p;
                    }
                }
            }
    ...
    Schematic.prototype.catsoop_transient_analysis = function(npts, tstop) {
        var ckt = this.extract_circuit(true);
        if (ckt === null){
            this.transient_results = null;
            return;
        }
        npts = ckt.parse_number(npts);
        tstop = ckt.parse_number(tstop);
        var results = ckt.tran(npts,
                               0,
                               tstop,
                               [],
                               false);
            ...
            if (this.submit_analyses != undefined) {
                ...
                if (submit != undefined) {
                    ...
                    for (var i in results){
                        ...
                        if (results[i] instanceof Array){
                            this.transient_results[i] = results[i];
                        }else{
                            this.transient_results[i] = new Array(this.transient_results.time.length).fill(results[i]);
                        }
                    }
                }
            }
	...
	Schematic.prototype.transient_analysis = function(silent) {
        silent = (typeof(silent) == 'undefined') ? false : silent;
	    this.unselect_all(-1);
	    this.redraw_background();
    ...
    Schematic.prototype.redraw = function(showResults) {
        showResults = (typeof(showResults) == 'undefined') ? false : showResults;
        // remove any previous annotations
        ...
        if (this.operating_point && showResults) {
        ...
        if (x2 != undefined) {
            ...
            if (graph.y_values != undefined) {
                    ...
                    if (values == undefined || color == undefined)
                        continue;  // no data points or x-axis
    ...
    function redraw_plot(graph) {
        ...
        if (graph.cursor1_x != undefined)
            plot_cursor(c,graph,graph.cursor1_x,4);
        if (graph.cursor2_x != undefined)
            plot_cursor(c,graph,graph.cursor2_x,30);
    }
    ...
    Component.prototype.rotate = function(amount) {
        ...
        // TODO: create an undoable edit record here
        // using old_rotation
    }
    ...
    Component.prototype.draw_arc = function(c,x,y,radius,start_radians,end_radians) {
        ...
        var nx = this.transform_x(x,y) + this.x;
        var ny = this.transform_y(x,y) + this.y;
        this.sch.draw_arc(c,nx,ny,radius,start_radians+rot_angle[this.rotation],
                          end_radians+rot_angle[this.rotation],false,1,false);
    }
    ...
    Component.prototype.edit_properties = function(x,y) {
        if (this.near(x,y)) {
            ...
            var content = build_table(fields, this.units);
            content.fields = fields;
    ...
    Wire.prototype.other_end = function(cp) {
        if (cp == this.connections[0])
            return this.connections[1];
        else if (cp == this.connections[1])
    ...
    function Label(x,y,rotation,label) {
        Component.call(this,'b',x,y,rotation);
    ...
    Ammeter.prototype.display_current = function(c,vmap) {
        ...
        if (v != undefined) {
            ...
            delete vmap[label]; // only display each current once
        }
    }
