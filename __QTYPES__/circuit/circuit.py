...
PARTS_MAP = {
    'ground': 'g', 'gnd': 'g', 'g': 'g',
    'label': 'b', 'b': 'b',
    'voltage': 'v', 'vsrc': 'v', 'voltage source': 'v', 'v': 'v',
    'current': 'i', 'isrc': 'i', 'current source': 'i', 'i': 'i',
    'resistor': 'r', 'r': 'r',
    'capacitor': 'c', 'c': 'c',
    'inductor': 'l', 'l': 'l',
    'opamp': 'o', 'o': 'o',
    ...
    'nfet': 'n', 'n': 'n',
    'pfet': 'p', 'p': 'p',
    'probe': 's', 's': 's', 'vprobe': 's',
    'ammeter': 'a', 'iprobe': 'a', 'a': 'a'
}
...
def create_check_rep(x, info):
    if isinstance(x, dict) and all(i in x for i in {'circuit', 'results', 'vprobes', 'iprobes'}):
        return x
...
def render_html(last_log, **info):
    ...
    out += ' submit_analyses="%s"' % escape(json.dumps({k: True for k in info['csq_check_analyses']}))
