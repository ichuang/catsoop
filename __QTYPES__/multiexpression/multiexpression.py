...
def get_parsed_reps(submissions, **info):
    ...
    for (ix, (prompt, solutions)) in enumerate(info['csq_expressions']):
        ...
        try:
            ...
            parsed.append('%s<br/><displaymath>%s</displaymath>' % (fprompt, expression['tree2tex'](info, funcs, sub)[0]))
        except:
            parsed.append('%s</br><center><font color="red">Error: could not parse your expression <code>%s</code></center></font>' % (fprompt, repr(osub)))
    ...
    for (ix, (prompt, solutions)) in enumerate(info['csq_expressions']):
        ...
        for soln in solutions:
            try:
                ...
                parsed.append('%s<br/><displaymath>%s</displaymath>' % (fprompt, expression['tree2tex'](info, funcs, soln)[0]))
            except:
                parsed.append('%s</br><center><font color="red">Error: could not parse expression <code>%s</code></center></font>' % (fprompt, repr(soln)))
