...
def sandbox_run_code(context, code, options):
    ...
    sandbox_file = os.path.join(context['cs_fs_root'], '__QTYPES__',
                                'pythoncode', '__SANDBOXES__', '%s.py' % s)
...
def fix_error_msg(fname, err, offset, sub):
    ...
    def subber(m):
        ...
        if lineno > (offset + sublen + 1):
            ...
            out += 'File <User-Submitted Code>, line %d%s' % (lineno - offset,
                                                              g[2])
        return out
...
def sandbox_run_test(context, code, test):
    ...
    options.update(test.get('options', {}))
...
def safety_check(code, bad_imports=None, bad_variables=None):
    ...
    vars = search(
        tree,
        lambda n: (isinstance(n, ast.Name) and (not isinstance(n._hz_parent, ast.Attribute)) and (not (isinstance(n._hz_parent, ast.Assign) and n in n._hz_parent.targets))))
    ...
    imports = _ast_downward_search(
        tree,
        lambda n: (isinstance(n, ast.Import) or isinstance(n, ast.ImportFrom)))
